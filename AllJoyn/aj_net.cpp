...
#endif
*/
static Adafruit_CC3000_Client g_client;
static Adafruit_CC3000_Client g_clientUDP;
...
AJ_Status AJ_Net_Connect(AJ_NetSocket* netSock, uint16_t port, uint8_t addrType, const uint32_t* addr)
{
	...
	g_client = cc3000.connectTCP(ip, port);
...
AJ_Status AJ_Net_MCastUp(AJ_NetSocket* netSock)
{
	...
	directedBcastAddr = (uint32_t(netmask) & uint32_t(ipAddress)) | (~uint32_t(netmask));
        ...
        Serial.print(F("\nIP directedBcastAddr: ")); cc3000.printIPdotsRev(directedBcastAddr);
     }
		
		
	//for Bcast address and port
	g_clientUDP = cc3000.connectUDP(directedBcastAddr, AJ_UDP_PORT);
        if (g_clientUDP.connected()) 
        {
              Serial.print(F("alljoyn cc3000 connected...."));
        }
